<?php
// pm25_realtime_real.php - แสดงข้อมูล PM2.5 แบบ realtime (ใช้ CID จริง)
require_once __DIR__ . '/config/database.php';
$pdo = getPDO();
// กำหนด CID จริง
$cids = ["E465B875ADCC", "E86BEAF6EB60"];
// ดึงข้อมูลล่าสุดต่อ sensor (เฉพาะ CID จริง)
$in = str_repeat('?,', count($cids)-1) . '?';
$stmt = $pdo->prepare("SELECT t1.* FROM pm25_data t1 INNER JOIN (SELECT cid, MAX(sensor_timestamp) AS max_ts FROM pm25_data WHERE cid IN ($in) GROUP BY cid) t2 ON t1.cid = t2.cid AND t1.sensor_timestamp = t2.max_ts ORDER BY t1.cid");
$stmt->execute($cids);
$latest = $stmt->fetchAll();
// ดึงข้อมูลย้อนหลัง 12 ชั่วโมง (สำหรับกราฟ เฉพาะ CID จริง)
$stmt2 = $pdo->prepare("SELECT * FROM pm25_data WHERE cid IN ($in) AND sensor_timestamp >= UNIX_TIMESTAMP(NOW())-43200 ORDER BY sensor_timestamp ASC, cid ASC");
$stmt2->execute($cids);
$rows = $stmt2->fetchAll();
$chartData = [];
foreach ($rows as $row) {
...
<head>
    <meta charset="UTF-8">
    <title>PM2.5 Dashboard (Real)</title>
...
<body>
    <div class="container">
        <h2>PM2.5 Dashboard (Realtime - Real CID)</h2>
        <div class="dashboard">
